...
- Generated sources are produced two ways.  meson owns the re2c, bison and VM codegen: a `custom_target` in each
  directory's `meson.build` regenerates them, with the exact flags `build/regenerate` uses (so the result
  matches autoconf's byte for byte apart from `#line` source paths), and a meson build never needs `just
  generate`.  The re2c/bison outputs are gitignored and generated into the build directory.  The VM outputs
  (`zend_vm_opcodes.h`/`.c`, `zend_vm_execute.h`, `zend_vm_handlers.h`) are committed — upstream regenerates
  them constantly, so un-committing them to generate into the build tree would make every merge fight the
  deletion — so meson runs `zend_vm_gen.php` in place via `find_program('php')` and records a stamp.
  `build/regenerate` (via `just generate`) stays for autoconf, which still needs the source-tree copies;
  touching any `*.l`, `*.y`, `*.re` or `Zend/zend_vm_def.h` is picked up by the next `just meson`.

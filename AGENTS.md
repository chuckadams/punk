...
meson-only tree needs neither `just configure` nor `just generate`: re2c and bison run inside the build, the
VM regenerates in place, and the config headers are generated by meson.
...
- Generated sources are all gitignored build products, owned by meson: a `custom_target` in each directory's
  ...
  The re2c/bison outputs are generated into the build directory.  The VM outputs (`zend_vm_opcodes.h`/`.c`,
  `zend_vm_execute.h`, `zend_vm_handlers.h`) have no output-directory option in `zend_vm_gen.php`, so meson
  runs it in place via `find_program('php')` and records a stamp.  `build/regenerate` (via `just generate`)
  stays for autoconf, which still needs the source-tree copies; touching any `*.l`, `*.y`, `*.re` or
  `Zend/zend_vm_def.h` is picked up by the next `just meson`.  (Upstream commits the VM files, so an upstream
  merge that touches them will conflict with punk's deletion; the resolution is to regenerate them.)

...
meson-only tree needs neither `just configure` nor `just generate`: re2c/bison run inside the build, the VM
files are committed, and the config headers are generated by meson.
...
1. **The m4 stack is still there, for phpize.** meson no longer borrows anything from autoconf — it generates
   every config header and every re2c/bison source into the build directory — but `just configure`/`make`/
   `test`/`all` remain as the autotools path, and `config.m4` + `phpize` have no meson equivalent yet.
   Deleting `configure.ac`, the `build/*.m4` files, `buildconf` and `Makefile.global` waits on that.
...
- Generated sources are produced two ways.  meson owns the re2c and bison outputs: a `custom_target` in each
  directory's `meson.build` regenerates them into the build directory (with the exact flags `build/regenerate`
  uses, so the result matches autoconf's byte for byte apart from `#line` source paths), so a meson build never
  needs `just generate`.  The VM outputs (`zend_vm_opcodes.h`/`.c`, `zend_vm_execute.h`) are committed and still
  come from `php Zend/zend_vm_gen.php`, which meson does not own because it needs a php interpreter;
  `build/regenerate` (via `just generate`) produces them for both autoconf and meson.  Run `just generate` after
  touching `Zend/zend_vm_def.h`; touching `*.l`, `*.y` or `*.re` is picked up by the next `just meson`.

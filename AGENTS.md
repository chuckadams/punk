...
| `sapi/` | SAPI modules; `cli`, `cgi`, `embed`, `fpm` and `phpdbg` have meson builds, the rest are upstream-only so far |
...
- `platform/<triple>/.env` is git-ignored — copy `.env.example` on a fresh clone. It supplies `CC`/`CXX`
  ...
  meson build builds all five front-ends (cli, cgi, embed, fpm, phpdbg) unconditionally.

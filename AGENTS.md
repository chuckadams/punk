...
| `sapi/` | SAPI modules; `cli`, `cgi` and `embed` have meson builds, the rest are upstream-only so far |
...
- Install layout: `/opt/punk/bin/punk`, `bin/php-cgi`, `bin/php-config` (generated by `scripts/meson.build`
  from upstream's `php-config.in`), the embed library as `/opt/punk/lib/libphp.{so,dylib}`, shared modules
  flat in `/opt/punk/lib/*.so`, headers under
  `/opt/punk/include/php/{main,streams,Zend,TSRM,sapi,ext/…}`, and `share/man/man1/php-cgi.1`. `php-config` answers
